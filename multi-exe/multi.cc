...
[[noreturn]] void help(const std::string& cmd, int exit_status = EXIT_FAILURE) {
    std::cerr << "Usage:\n";
    std::cerr << '\t' << cmd << " input.csv [chunk_size] [num_workers]\n";
    std::cerr << "\t\t" << "input.csv: file containing time series\n";
    std::cerr << "\t\t" << "chunk_size: how many lines per chunk\n";
    std::cerr << "\t\t" << "num_workers: how many workes to spawn\n";
    exit(exit_status);
}
...
void spawn_workers(std::size_t ts_count, unsigned int n = std::thread::hardware_concurrency()) {
...
int main(int argc, char const *argv[])
{
    ...
    if (argc == 4) {
        try {
            auto tmp = std::stol(argv[3]);
            if (tmp <= 0) {
                throw std::exception();
            }
            n_workers = tmp;
        } catch(const std::exception& e) {
            std::cerr << "n_workers must be a positive integer, found '" << argv[3] << "'" << '\n';
            help(argv[0], EXIT_FAILURE);
        }
    }
    if (argc > 4) {
    ...
    spawn_workers(ts_count, n_workers);
    while (true) {
