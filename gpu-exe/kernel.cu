...
[[noreturn]] void help(const std::string& cmd, int exit_status = EXIT_FAILURE) {
    std::cerr << "Usage:\n";
    std::cerr << '\t' << cmd << " input.csv [blocks] [threads-per-block]\n";
...
struct PCC_Partial {
    ...
    data_type sum_1{};
    data_type sum_2{};
    data_type sum_1_squared{};
    data_type sum_2_squared{};
    data_type sum_prod{};
};
...
std::vector<thrust::device_vector<data_type>>& get_chunk(csv::reader& r, std::size_t line_count) {
    static auto column_count = r.column_count();
    static std::vector<thrust::host_vector<data_type>> tmp(column_count);
    // many columns as tmp
    static auto ans = std::vector<thrust::device_vector<data_type>>(column_count);
...
// idea: input-file [block number: 16] [thread-per-block number: 1024]
//  cuda_fun<<<block number,thread-per-block>>>
int main(int argc, char const* argv[])
{
	...
	uint blocks = 16, threads_per_block = 1024;
	if (argc == 4) {
		blocks = math::convertions::ston<uint>(argv[2]);
		threads_per_block = math::convertions::ston<uint>(argv[3]);
	}
...
#ifndef NDEBUG
        std::cerr << "Processing rows [" << precessed_rows << ',' << (precessed_rows+=chunk[0].size()) << ")...";
#endif
        evaluate<<<blocks,threads_per_block>>>(thrust::raw_pointer_cast(&partial[0]), thrust::raw_pointer_cast(&c[0]), ts_count, chunk[0].size());
#ifndef NDEBUG
        std::cerr << " DONE!\n";
#endif
    }
...
#endif
    compute_results<<<blocks,threads_per_block>>>(ts_count, thrust::raw_pointer_cast(&res[0]), thrust::raw_pointer_cast(&partial[0]));
#ifndef NDEBUG
    std::cerr << " DONE!\n";
#endif
