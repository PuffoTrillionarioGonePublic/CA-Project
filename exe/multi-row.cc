...
std::vector<math::statistics::multicolumn_pcc_accumulator<data_type>> results;

// called by workers before terminating
void store_worker_result(math::statistics::multicolumn_pcc_accumulator<data_type>&& partial_res) {
...
void spawn_workers(std::size_t ts_count) {
    ...
    for (decltype(n) i{}; i != n; ++i) {
        workers.emplace_back([ts_count](){
            // now per thread
            math::statistics::multicolumn_pcc_accumulator accumulator(ts_count);
            ...
            // store results
            store_worker_result(std::move(accumulator));
        });
    }
}
...
int main(int argc, char const *argv[])
{
    ...
    print_results(join_all().results());
